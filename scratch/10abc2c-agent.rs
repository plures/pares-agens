...
impl Agent {
    ...
    ///
    /// When set, every userΓåÆassistant exchange is persisted as a [`ChatTurn`]
    ...
    pub async fn handle_event(&self, event: Event) -> Option<Event> {
        ...
        // ΓöÇΓöÇ Cerebellum: autorecall + routing ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        let model_client = match &self.model_client {
            ...
            None => {
                ...
                return Some(Event::ModelResponse {
                    ...
                    content: "ΓÜá∩╕Å Model client not configured.".into(),
                });
            }
        };
        let tool_dispatcher = match &self.tool_dispatcher {
            ...
            None => {
                ...
                return Some(Event::ModelResponse {
                    request_id: id.to_string(),
                    model: "unconfigured".into(),
                    content: "ΓÜá∩╕Å Tool dispatcher not configured.".into(),
                });
            }
        };
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return Some(Event::ModelResponse {
                    request_id: id.to_string(),
                    model: "error".into(),
                    content: format!("ΓÜá∩╕Å Model error: {e}"),
                });
            }
        };
    ...
    // ΓöÇΓöÇ Conversation history persistence ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    fn trim_to_token_budget(messages: &[ChatMessage]) -> Vec<ChatMessage> {
        ...
        // Exceeds budget ΓÇö keep most recent messages that fit
    ...
    fn extract_facts(&self, response: &str) -> Vec<String> {
        response
            ...
            .map(|s| s.trim().trim_start_matches(['-', '*', 'ΓÇó']))
            .filter(|s| !s.is_empty())
...
mod tests {
    ...
    // ΓöÇΓöÇ Cerebellum-aware agent tests ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    async fn agent_with_cerebellum_returns_response_for_conscious_route() {
        ...
        // Short message ΓåÆ Conscious route ΓåÆ response returned.
    ...
    async fn agent_with_cerebellum_drops_noise_messages() {
        let agent = make_agent_with_cerebellum();
        // Single-word ack "ok" ΓåÆ Route::Drop ΓåÆ None.
    ...
    // ΓöÇΓöÇ Headroom seam: run_model_loop direct drive ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    //
    ...
    async fn run_model_loop_compresses_transient_clone_preserving_canonical() {
        ...
        assert_eq!(calls.len(), 1, "single turn ΓåÆ single model call");
    ...
    async fn run_model_loop_without_headroom_sends_originals() {
        ...
        let tools: Arc<dyn ToolDispatcher> = Arc::new(MockTools);
        // No `.with_headroom(...)` ΓåÆ None path.
        let agent = Agent::new(Arc::new(InMemory::new())).with_model(
            Arc::clone(&model),
            Arc::clone(&tools),
            "sys".into(),
