...
- Direct fix flow: clone/use cached repo copy → read CI error output → fix code with `edit_file` → run repo-native checks (e.g., `cargo check`/`cargo clippy` for Rust) → commit + push (same PR branch)
