...
- Two sibling instances of the identical "Rust fallback until PxBridge wires fully" pattern exist elsewhere with the same caveat, never resolved: `crates/core/src/orchestrator/actions.rs:558` and `crates/agens-plugin/src/agent_commands/runtime.rs:4785-4786`.
- PxBridge itself is real, tested, and wired elsewhere (`crates/core/src/orchestrator/px_bridge.rs`; constructed at `runtime.rs:316,5273`; threaded through `Orchestrator` at `orchestrator/mod.rs:186,236`) — so the intended integration point is provably reachable, it's just never connected to promise detection.
