...
fn task_transition_allowed(from: &str, to: &str) -> bool {
    VALID_TASK_TRANSITIONS.iter().any(|(f, t)| *f == from && *t == to)
}
...
impl Rule for TaskIdPresent {
    fn name(&self) -> &str { "task_id_present" }
...
impl Rule for TaskTypeKnown {
    fn name(&self) -> &str { "task_type_known" }
    fn category(&self) -> RuleCategory { RuleCategory::Input }
    fn evaluate(&self, ctx: &RuleContext) -> RuleResult {
...
impl Rule for RequiredCapabilityDeclared {
    fn name(&self) -> &str { "required_capability_declared" }
    fn category(&self) -> RuleCategory { RuleCategory::Input }
    fn evaluate(&self, ctx: &RuleContext) -> RuleResult {
        match ctx.payload_array_len("required_capabilities") {
            None | Some(0) => RuleResult::Fail {
                reason: "task must declare at least one required_capability".into(),
            },
            _ => RuleResult::Pass,
        }
    }
...
impl Rule for TaskStateTransitionValid {
    fn name(&self) -> &str { "task_state_transition_valid" }
...
impl Rule for AssignedAgentPresentWhenInProgress {
    fn name(&self) -> &str { "assigned_agent_present_when_in_progress" }
    fn category(&self) -> RuleCategory { RuleCategory::State }
    fn evaluate(&self, ctx: &RuleContext) -> RuleResult {
...
impl Rule for PriorityInRange {
    fn name(&self) -> &str { "priority_in_range" }
...
impl Rule for LoadBalanceHeadroom {
    fn name(&self) -> &str { "load_balance_headroom" }
    fn category(&self) -> RuleCategory { RuleCategory::Data }
    fn evaluate(&self, ctx: &RuleContext) -> RuleResult {
...
mod tests {
    ...
    fn module() -> TaskRoutingModule { TaskRoutingModule::default() }
    ...
    fn task_id_present_passes() {
        ...
        let r = results.iter().find(|(n, _)| n == "task_id_present").unwrap();
    ...
    fn task_type_known_passes_inference() {
        ...
        let results = module().evaluate_category(&ctx, RuleCategory::Input);
        let r = results.iter().find(|(n, _)| n == "task_type_known").unwrap();
        assert_eq!(r.1, RuleResult::Pass);
    }
    ...
    fn task_transition_queued_to_assigned_passes() {
        ...
        let r = results.iter().find(|(n, _)| n == "task_state_transition_valid").unwrap();
        assert_eq!(r.1, RuleResult::Pass);
    }
    ...
    fn assigned_agent_required_for_in_progress() {
        ...
        let results = module().evaluate_category(&ctx, RuleCategory::State);
        let r = results.iter().find(|(n, _)| n == "assigned_agent_present_when_in_progress").unwrap();
        assert!(matches!(r.1, RuleResult::Fail { .. }));
    }
    ...
    fn priority_in_range_passes() {
        ...
        let r = results.iter().find(|(n, _)| n == "priority_in_range").unwrap();
        assert_eq!(r.1, RuleResult::Pass);
    }
    ...
    fn load_balance_warns_at_high_utilisation() {
        let ctx = RuleContext::new("load_check", json!({"queue_depth": 90, "agent_capacity": 100}));
        let results = module().evaluate_category(&ctx, RuleCategory::Data);
        let r = results.iter().find(|(n, _)| n == "load_balance_headroom").unwrap();
        assert!(matches!(r.1, RuleResult::Warning { .. }));
    }

    #[test]
    fn load_balance_passes_below_80_pct() {
        let ctx = RuleContext::new("load_check", json!({"queue_depth": 50, "agent_capacity": 100}));
        let results = module().evaluate_category(&ctx, RuleCategory::Data);
        let r = results.iter().find(|(n, _)| n == "load_balance_headroom").unwrap();
        assert_eq!(r.1, RuleResult::Pass);
    }
    ...
    fn module_audit_is_complete() {
        ...
        assert!(report.is_complete(), "task-routing should cover all categories");
