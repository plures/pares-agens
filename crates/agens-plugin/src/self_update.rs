...
//!
//! RELOCATED (B1 Option A, Stage R2): copied verbatim from
//! `crates/agenda/src/self_update.rs` into the agens host crate (`agens-plugin`)
//! so the host owns its self-update behavior. The only edit vs. the agenda copy
//! is the scheduler path: in the agenda crate `scheduler` is a sibling module
//! (`super::scheduler`), whereas here it is reached through the
//! `pares_agens_agenda` dependency (`pares_agens_agenda::scheduler`).
...
/// Build a [`pares_agens_agenda::scheduler::Task`] for periodic self-update.
...
pub fn self_update_task_from_env() -> pares_agens_agenda::scheduler::Task {
    let flake_dir =
