//! Persistent configuration file support for the pares-agens host runtime.
//!
//! RELOCATED (Stage R3a) from `pares-radix-cli-runtime/src/config.rs` into
//! `agens-plugin` as part of folding the `run_with_providers` host composition
//! seam into the host crate (Option A: agens-plugin IS the host). Behavior is
//! unchanged from the radix original.
//!
//! Config file location: `~/.config/pares-radix/config.toml` (XDG standard).
//! CLI args and env vars override config values.
//!
//! NOTE (deferred follow-up): the on-disk config dir is still named
//! `pares-radix` (and the data/log dirs are `~/.pares-radix`). Renaming the
//! on-disk directory to `pares-agens` is a behavior change deliberately left
//! OUT of scope for R3a; track it as a follow-up.
