fn main() {
    ...
    // crate (ADR-0010: no duplicated operational logic). In sandboxed builds
    // (Nix, Docker) with no git access the fallback IS the release version from
    // the tag that triggered the build.
