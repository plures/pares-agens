...
/** Probe Docker Model Runner and update the badge. */
async function runDockerDetect() {
  dockerStatus.textContent = "Detecting…";
  ...
  try {
    ...
    if (found) {
      ...
      dockerStatus.textContent = "Not detected";
      dockerStatus.className = "wizard-badge badge-warn";
    }
  } catch (_) {
    dockerStatus.textContent = "Check failed";
    dockerStatus.className = "wizard-badge badge-warn";
  }
}
...
validateBtn.addEventListener("click", async () => {
  ...
  try {
    const valid = await invoke("validate_api_key", { provider, apiKey: key });
    if (valid) {
      ...
      apiKeyStatus.textContent = "✗ Key rejected — please check and retry.";
...
document.getElementById("wizard-next-1").addEventListener("click", () => {
  // Cloud source requires a (validated) key before proceeding
  if (state.modelSource === "cloud" && !state.apiKey) {
    apiKeyStatus.textContent = "Please enter and validate an API key, or choose a different source.";
    apiKeyStatus.className = "wizard-hint hint-warn";
    return;
  }
  goNext();
...
async function finishWizard() {
  ...
  try {
    ...
    try { await invoke("set_settings", { settings }); } catch (_) {}
  }
