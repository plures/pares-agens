...
/// during the wizard flow. `agent_name` is used as the display name in the
/// chat header (stored in `settings.channel` label for now and as the system
/// prompt prefix when the system prompt is empty).
#[tauri::command]
