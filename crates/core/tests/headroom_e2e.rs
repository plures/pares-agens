...
fn load_px(name: &str) -> String {
    // Fixtures live in the workspace `praxis/` tree, committed in-repo.
    // Resolve relative to this crate (crates/core) so the test is hermetic
    // and runs anywhere (incl. the sandboxed nix build), not just one dev box.
