        ...
        match self.validate(key).await {
            ...
            Err(LicenseError::NetworkError(_)) => {
                if let Some(last) = last_validated_at {
                    let cutoff = Utc::now()
                        - chrono::TimeDelta::days(self.grace_days);
                    if last > cutoff {
...
mod tests {
    ...
    async fn polar_validator_grace_period_allows_offline_within_window() {
        // Point at a non-routable address to force a network error.
        let validator = PolarValidator::new("test-benefit-id");
        // Override internal client to guarantee a connection failure.
        // We simulate the network-error branch directly via a surrogate that
        // always errors.
        ...
        // Replicate the logic without making a real HTTP call.
    ...
    fn polar_validator_empty_key_returns_invalid() {
        // The guard at the top of PolarValidator::validate catches empty keys
        // synchronously (without network access).  Verify via FixedKeyValidator
        // which shares the same empty-key contract.
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            let validator = FixedKeyValidator::new("key");
            let err = validator.validate("").await.unwrap_err();
            assert!(matches!(err, LicenseError::InvalidKey { .. }));
        });
    }
